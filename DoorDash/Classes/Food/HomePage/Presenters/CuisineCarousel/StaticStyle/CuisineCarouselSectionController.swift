...
final class CuisineCarouselSectionController: ListSectionController, ListAdapterDataSource {
    ...
    override func sizeForItem(at index: Int) -> CGSize {
        ...
        if index == 0 {
            height = BrowseFoodSectionHeaderViewCell.calcHeight(containerWidth: width, title: "CUISINES")
        } else {
        ...
        controller.didSelectCuisine = { cuisine in
            for page in self.pages?.pages ?? [] {
                for item in page.items {
                    item.selected = cuisine.id == item.cuisine.id
                    print(item.selected)
                }
            }
